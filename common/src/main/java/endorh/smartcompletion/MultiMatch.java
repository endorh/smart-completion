...
import static endorh.smartcompletion.util.PolyFill.compareArrays;
...
public class MultiMatch implements Comparable<MultiMatch> {
    ...
    */
   public MultiMatch(
      WordSplit split, String[] parts, int[] indices,
      int[] partIndices, int[][] repeats, int priority
   ) {
   ...
   /** Word split of the suggestion. */
   public WordSplit split() { return split; }
   /** Split of the query into matched segments. */
   public String[] parts() { return parts; }
   /** Indices within the suggestion where each of the {@code parts} matched. */
   public int[] indices() { return indices; }
   /** Indices within the split of the suggestion where each of the {@code parts} matched. */
   public int[] partIndices() { return partIndices; }
   /** Arrays of extra indices where each of the {@code parts} could've alternatively been matched. */
   public int[][] repeats() { return repeats; }
   /** Priority of the match. Matches with higher priority are preferred. */
   ...
   @Override public int compareTo(@NotNull MultiMatch other) {
      ...
      if ((r = compareArrays(partIndices, other.partIndices)) != 0) return r;
