...
import static endorh.smartcompletion.util.PolyFill.empty;
import static endorh.smartcompletion.util.PolyFill.literal;
...
public class SmartCommandCompletion {
   ...
   public static AggregatedSuggestions filterAndSort(
      ...
      if (argQuery.isEmpty()) {
         ...
         List<MultiMatch> matches = suggestions.stream().map(s -> MultiMatch.whole(s.getText())).collect(Collectors.toList());
   ...
   public static MutableComponent highlightSuggestion(String suggestion, MultiMatch matches, String query) {
      ...
      MutableComponent builder = empty();
      ...
      for (int i = 0; i < matches.size(); i++) {
         ...
         ).withStyle(matches.isWeak() ? style.weak_match.get() : style.match.get()));
