![Smart Completion Mod](https://github.com/endorh/smart-completion/blob/main/images/%5Bg%5Dolden_%5Bap%5Dple.png?raw=true)
...
with the partially typed argument (informed) and one without (blind).

This mod relies on the server replying with all possible suggestions to the
blind request.
...
suggestions by matching your partially typed argument against the initials
of the words in each suggestion.
...
#### Smart Matching
The smart matching algorithm splits both your query and a suggestion in
*parts* or *words*. Then, the algorithm will try to split each part of your
query in a way that can match the initials of the parts of the suggestion
in order, disregarding case.

In general, you won't need to type different multiple words in your query,
but it may be useful to enforce a specific split.
...
- original order from the server
...
The currently selected suggestion is highlighted in yellow, which
